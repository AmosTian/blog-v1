...
>   A的补$$\overline{A}$$ ：$$\overline{A} = U-A = \{x|x\in U∧ x\notin A\}$$
...
>   C的成员的并，记为 $$U_{S\in C}S = \{x|∃x(S\in C∧x\in S)\}$$ 

-   如果 $$x\in U_{S\in C}$$ ，那么x至少是一个子集S的元素

>   若 $$C\neq∅$$ ，C的成员的交，记为 $$∩_{S\in C}=\{x|\forall x(S\in C\rightarrow x\in S)\}$$
...
>
>   搜集 $$C=\{A_d|d\in D\}$$ 为加索引搜集，D为搜集的索引集合

例：

$$C=P\{A_0,A_1,A_2,...,A_n\}$$ ，则 C的成员的并 $$U_{S\in C}S$$ 记为
